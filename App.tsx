...
// Endpoint de la nube (KVDB.io) - Servidor de persistencia rápida
...
const App: React.FC = () => {
  ...
  const handleImportAllData = useCallback((data: any) => {
    ...
    if (data.schoolSettings) {
      setSchoolSettings(prev => ({
        ...prev,
        ...data.schoolSettings,
        cloudProjectKey: prev.cloudProjectKey || data.schoolSettings.cloudProjectKey, // Preservar la llave local si ya existe
  ...
  const syncWithCloud = useCallback(async (push = false, forceKey?: string) => {
    ...
    try {
      if (push) {
        ...
        if (!response.ok) throw new Error("Fallo al subir");
        ...
        } else if (response.status === 404 && push) {
          // Si el registro no existe, forzamos una subida inicial
          await syncWithCloud(true, key);
        }
      }
    } catch (e) {
  ...
  // Manejo de activación forzada desde Configuración
  const handleCloudActivation = useCallback(async (newKey: string) => {
    setSchoolSettings(prev => ({ ...prev, cloudProjectKey: newKey }));
    // Pequeño delay para asegurar que el estado se actualice antes de subir
    setTimeout(() => {
        syncWithCloud(true, newKey);
    }, 500);
  }, [syncWithCloud]);
  ...
  useEffect(() => { if (isDataLoaded) { triggerSave('students', students); setHasUnsavedChanges(true); } }, [students, triggerSave, isDataLoaded]);
  useEffect(() => { if (isDataLoaded) { triggerSave('teachers', teachers); setHasUnsavedChanges(true); } }, [teachers, triggerSave, isDataLoaded]);
  useEffect(() => { if (isDataLoaded) { triggerSave('payments', payments); setHasUnsavedChanges(true); } }, [payments, triggerSave, isDataLoaded]);
  useEffect(() => { if (isDataLoaded) { triggerSave('cashFlow', cashFlow); setHasUnsavedChanges(true); } }, [cashFlow, triggerSave, isDataLoaded]);
  useEffect(() => { if (isDataLoaded) { triggerSave('squads', squads); setHasUnsavedChanges(true); } }, [squads, triggerSave, isDataLoaded]);

  const applyRemoteUpdate = async () => {
    setIsSyncing(true);
    try {
      const response = await fetch(`${CLOUD_API_BASE}${schoolSettings.cloudProjectKey}`);
      if (response.ok) {
      ...
      setSyncError("Fallo al descargar");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (!hasUnsavedChanges && isOnline) syncWithCloud(false);
    }, 45000);
    return () => clearInterval(interval);
  }, [syncWithCloud, hasUnsavedChanges, isOnline]);
        ...
        <div className="p-6 h-full flex flex-col">
          ...
          <div className="mt-auto pt-6 border-t border-slate-800 text-center">
             <div className="bg-white/5 p-4 rounded-2xl mb-4 text-left border border-white/5">
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">ID PROYECTO</p>
          ...
          <div className="flex items-center gap-3">
             {remoteUpdateAvailable && (
               <button onClick={applyRemoteUpdate} className="flex items-center gap-2 bg-amber-500 text-white px-4 py-2 rounded-full text-[10px] font-black uppercase animate-bounce shadow-lg">
                 <Zap className="w-3.5 h-3.5" /> Cambios detectados
               </button>
             )}
             
             <div className="flex items-center gap-2">
               {syncError && (
                 <div className="flex items-center gap-1.5 text-red-600 text-[10px] font-black uppercase bg-red-50 px-3 py-2 rounded-full border border-red-100 animate-pulse">
               ...
               {schoolSettings.cloudProjectKey && isOnline && (
                 <button 
                  ...
                  disabled={isSyncing} 
                  className={`flex items-center gap-2 font-black text-[10px] px-5 py-2.5 rounded-full transition shadow-lg ${hasUnsavedChanges || syncError ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-100'}`}
                >
                    {isSyncing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (hasUnsavedChanges || syncError ? <CloudUpload className="w-3.5 h-3.5" /> : <CloudCheck className="w-3.5 h-3.5" />)}
                    {isSyncing ? 'CONECTANDO' : (hasUnsavedChanges || syncError ? 'SUBIR A NUBE' : 'SINCRONIZADO')}
                  </button>
               )}
             </div>
          </div>
