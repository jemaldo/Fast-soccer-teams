                ...
                <div className="lg:col-span-3 space-y-10">
                  ...
                  <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div><label className="block text-[9px] font-black text-slate-500 uppercase mb-2 px-1">Categoría</label><select name="category" defaultValue={selectedStudent?.category || schoolSettings.categories[0]} className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-sm font-black outline-none text-white">{schoolSettings.categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></div>
                      <div><label className="block text-[9px] font-black text-slate-500 uppercase mb-2 px-1">Posición de Juego</label><select name="position" defaultValue={selectedStudent?.position || schoolSettings.positions[0]} className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-sm font-black outline-none text-white">{schoolSettings.positions.map(pos => <option key={pos} value={pos}>{pos}</option>)}</select></div>
                      <div><label className="block text-[9px] font-black text-slate-500 uppercase mb-2 px-1">Tipo Entrenamiento</label><select name="trainingType" defaultValue={selectedStudent?.trainingType || 'Formativa'} className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-sm font-black outline-none text-white"><option value="Formativa">Formativa</option><option value="Elite">Elite / Rendimiento</option></select></div>
                      <div className="md:col-span-2"><label className="block text-[9px] font-black text-slate-500 uppercase mb-2 px-1">Docente / Coach Asignado</label><select name="teacherId" defaultValue={selectedStudent?.teacherId} className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-sm font-black outline-none text-white"><option value="">Sin docente asignado</option>{teachers.map(t => <option key={t.id} value={t.id}>{t.firstName} {t.lastName}</option>)}</select></div>
                      <div><label className="block text-[9px] font-black text-slate-500 uppercase mb-2 px-1">Fecha Ingreso</label><input type="date" name="entryDate" defaultValue={selectedStudent?.entryDate || new Date().toISOString().split('T')[0]} className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-sm font-bold outline-none text-white" /></div>
                    </div>
                  </div>
