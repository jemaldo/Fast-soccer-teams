...
const ReportManager: React.FC<Props> = ({ students, teachers, payments, cashFlow, schoolSettings }) => {
  ...
  const handlePrint = (type: string) => {
    // El navegador imprimirá lo que esté visible en .print-only
    setTimeout(() => {
      window.print();
    }, 100);
  };
  ...
  const categoryStudents = useMemo(() => 
    students.filter(s => s.category === selectedCategoryId),
  [selectedCategoryId, students]);

  const categoryDebtData = useMemo(() => {
    if (!selectedCategoryId) return [];
    return categoryStudents.map(student => ({
      student,
      debt: getStudentDebtStatus(student)
    }));
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200">
          <button 
            ...
            className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition ${selectedReport === 'INDIVIDUAL' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Individual
          </button>
          <button 
            onClick={() => setSelectedReport('CATEGORY')}
            className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition ${selectedReport === 'CATEGORY' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Por Categoría
          </button>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-200 shadow-xl overflow-hidden flex flex-col">
          <div className="p-8 border-b border-slate-100 bg-slate-50/50">
            <h4 className="text-xl font-black text-slate-800 flex items-center gap-3 mb-6 uppercase tracking-tighter">
              {selectedReport === 'INDIVIDUAL' ? (
                <><CalendarDays className="w-6 h-6 text-blue-600" /> Seguimiento Individual</>
              ) : (
                <><Layers className="w-6 h-6 text-purple-600" /> Estado de Cuenta por Categoría</>
              )}
            </h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedReport === 'INDIVIDUAL' ? (
                ...
                <div className="relative">
                  ...
                  <select 
                    className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-bold appearance-none outline-none focus:ring-2 focus:ring-purple-500 transition"
          ...
          <div className="p-8 flex-1 overflow-x-auto">
            {selectedReport === 'INDIVIDUAL' ? (
              ...
              /* REPORTE POR CATEGORIA (Nuevo) */
              !selectedCategoryId ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4 py-12">
                  <div className="bg-slate-50 p-6 rounded-full">
                    <Layers className="w-12 h-12 opacity-20" />
                  </div>
                  <p className="text-sm font-bold uppercase tracking-widest italic text-center">Selecciona una categoría para generar el reporte</p>
                </div>
              ) : (
                <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="flex justify-between items-center">
                    <div>
                      <h5 className="text-lg font-black text-slate-800 uppercase tracking-tight">Categoría: {selectedCategoryId}</h5>
                      <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">{categoryStudents.length} Alumnos Registrados</p>
                    </div>
                    <button 
                      onClick={() => handlePrint('CATEGORY_DEBT')}
                      className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition"
                    >
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-900 text-white">
                          <th className="p-3 font-black uppercase tracking-tighter sticky left-0 bg-slate-900 z-10">Alumno</th>
                          {monthsHeader.map(m => (
                            <th key={m} className="p-3 text-center font-black uppercase tracking-tighter">{m}</th>
                          ))}
                          <th className="p-3 text-center font-black uppercase tracking-tighter bg-slate-800">Debe</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {categoryDebtData.map(({ student, debt }) => (
                          <tr key={student.id} className="hover:bg-slate-50">
                            <td className="p-3 font-bold text-slate-800 sticky left-0 bg-white border-r shadow-[2px_0_5px_rgba(0,0,0,0.05)] whitespace-nowrap">
                              {student.fullName}
                            </td>
                            {debt?.monthsStatus.map((m, mIdx) => (
                              <td key={mIdx} className="p-2 text-center">
                                {m.isPaid ? (
                                  <div className="w-5 h-5 rounded-full bg-emerald-500 mx-auto flex items-center justify-center text-[8px] text-white font-black"><CheckCircle2 className="w-3.5 h-3.5" /></div>
                                ) : m.isBeforeEntry || m.isFuture ? (
                                  <div className="w-5 h-5 rounded-full bg-slate-100 mx-auto border border-slate-200"></div>
                                ) : (
                                  <div className="w-5 h-5 rounded-full bg-red-500 mx-auto flex items-center justify-center text-white"><AlertCircle className="w-3.5 h-3.5" /></div>
                                )}
                              </td>
                            ))}
                            <td className="p-3 text-center font-black text-red-600 bg-red-50/30">
                              {debt?.monthsOwed}
                            </td>
                          </tr>
                        ))}
                  ...
                  <div className="flex gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 uppercase">Pagado</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full bg-red-500"></div>
                      <span className="text-[10px] font-bold text-slate-500 uppercase">En Mora</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full bg-slate-200"></div>
                      <span className="text-[10px] font-bold text-slate-500 uppercase">No Aplica</span>
                    </div>
                  </div>
                </div>
              )
        ...
        {selectedReport === 'INDIVIDUAL' && selectedStudent && (
          <div className="space-y-8">
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
              <h2 className="text-xl font-black text-slate-800 mb-2 uppercase tracking-tighter">Alumno: {selectedStudent.fullName}</h2>
        ...
        {selectedReport === 'CATEGORY' && selectedCategoryId && (
          <div className="space-y-6">
             <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-6">
              <h2 className="text-xl font-black text-slate-800 mb-2 uppercase tracking-tighter">Reporte Consolidado: Categoría {selectedCategoryId}</h2>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Total Alumnos: {categoryStudents.length}</p>
            </div>

            <table className="w-full text-[9px] text-left border-collapse border border-slate-300">
              <thead>
                <tr className="bg-slate-100">
                  <th className="p-2 border border-slate-300 font-black uppercase">Alumno</th>
                  {monthsHeader.map(m => (
                    <th key={m} className="p-2 border border-slate-300 text-center font-black uppercase">{m}</th>
                  ))}
                  <th className="p-2 border border-slate-300 text-center font-black uppercase bg-slate-200">Debe</th>
                </tr>
              </thead>
              <tbody>
                {categoryDebtData.map(({ student, debt }) => (
                  <tr key={student.id}>
                    <td className="p-2 border border-slate-300 font-bold">{student.fullName}</td>
                    {debt?.monthsStatus.map((m, mIdx) => (
                      <td key={mIdx} className="p-2 border border-slate-300 text-center">
                        {m.isPaid ? 'SI' : m.isBeforeEntry || m.isFuture ? '-' : 'NO'}
                      </td>
                    ))}
            ...
            <div className="mt-6 flex gap-6 text-[10px] font-bold">
               <p>SI: Pagado</p>
               <p>NO: Pendiente</p>
               <p>-: No aplica / Futuro</p>
            </div>
          </div>
        )}

        <div className="mt-20 pt-10 border-t border-slate-200 text-center opacity-50">
           <p className="text-[10px] font-bold italic">Este es un documento oficial de control interno de {schoolSettings.name}</p>
